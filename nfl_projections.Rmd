...
from nfl_game_projections import *
import pandas as pd

if not (r.xgb): ypp_input = pd.DataFrame(r.data_mod)
#projections = nfl_model().projections(week=int(r.week),ypp_input = ypp_input)
# if r.xgb:
#   projections = nfl_model().projections(week=int(r.week))
# else:
#   projections = projections(week=r.week).run()
#   
projections = nfl_model().projections(week=int(r.week))
...
if (xgb==TRUE){
colnames(proj)[1]<-"home";colnames(proj)[2]<-"home_score";colnames(proj)[4]<-"away"
colnames(proj)[3]<-"away_score"
}else{
...
for (game in 1:nrow(proj)){
    ...
    proj$home_ypp[game]<- df_off$ypp[which(df_off$posteam == proj$home[game])] 
    proj$away_ypp[game]<-df_off$ypp[which(df_off$posteam == proj$away[game])] 
...
# show_data$Away_Proj[1]<-20.5

# show_data<-NFL_table_clean %>%
#   filter(Home == "Chiefs")

show_data$Home_Proj[1]<-21.0
show_data$Away_Proj[1]<-20.9
# show_data$Home_Proj[1]<-22.59
# show_data$Away_Proj[1]<-20.18
# show_data$Home_Proj[1]<-24.8
# show_data$Away_Proj[1]<-20.3
# show_data$Home_Proj[4]<-22.98
# show_data$Away_Proj[4]<-24.33
# 
```
...
table <-
    reactable(
      ...
      Home_EPA = team_rating_column(
          ...
          ### Normalize team rating in order to assign color from color palette
            normalized <-
              (value - min(unlist(agg$EPA))) / (max(unlist(agg$EPA)) - min(unlist(agg$EPA)))
        ...
        Home_EPA_Allowed = team_rating_column(
          ...
          cell = function(value) {
          ### Normalize team rating in order to assign color from color palette
            normalized <- (value - min(unlist(agg$EPA_allowed))) / 
               (max(unlist(agg$EPA_allowed)) - min(unlist(agg$EPA_allowed)))
            # 
      ...
      Home = colDef(
        ...
        cell = function(value, index) {
          ...
          div(class = "team", image,
            ...
            ### Team record
            div(class = "record",  sprintf("(%s)", show_data[index, "Home_Record"])))
      }
    ),
      Home_YPP = colDef(
          maxWidth = 55,
          ...
          style = function(value) {
            if (value > mean( c(proj$away_ypp,proj$home_ypp) )) {
              color <- "#008000"
            } else if (value < mean( c(proj$away_ypp,proj$home_ypp) )) {
              color <- "#e00000"
            } else {
              color <- "#777"
            }
    ...
    Home_Proj = colDef(
          ...
          ### For any positive number, assign green color. For any negative number assign red color
          style = function(value) {
            if (value > mean( c(proj$away_score,proj$home_score) ,na.rm = TRUE)) {
              color <- "#008000"
            } else if (value < mean( c(proj$away_score,proj$home_score) ,na.rm = TRUE)) {
              color <- "#e00000"
            } else {
              color <- "#777"
            }
      ...
      Away = colDef(
        ...
        cell = function(value, index) {
          ...
          div(class = "team", image,
            ...
            div(class = "team-name", value),
            ### Team record
            div(class = "record",  sprintf("(%s)", show_data[index, "Away_Record"])))
      }
    ),
      ...
      Away_EPA = team_rating_column(
          ...
          cell = function(value) {
          ### Normalize team rating in order to assign color from color palette
            normalized <-(value - min(unlist(agg$EPA))) / (max(unlist(agg$EPA)) - min(unlist(agg$EPA)))
            color <- off_rating_color(normalized)
            value <- format(value, nsmall = 1, digits = 1)
            ### Round corners of cell
            div(class = "roundcorners",
