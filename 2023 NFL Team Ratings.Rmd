...
for (row in 1:nrow(NFL_table_clean)){
  ...
  if (NFL_table_clean$Team[row] %in% word( model_pred$Home_Team,-1) ){
    ...
    #NFL_table_clean$Playoffs[row] = ifelse( home_spread <  away_spread, -abs(midpoint), abs(midpoint) )
    NFL_table_clean$Projection[row] = ifelse(home_spread < 0, paste("+",as.character(round(home_spread,1)),sep=""), paste("-",as.character(round(home_spread,1)),sep=""))
  }else{
    ht = model_pred$Home_Team[amatch(NFL_table_clean$Team[row] ,word( model_pred$Away_Team,-1), maxDist = 2)]
    at = model_pred$Away_Team[amatch(NFL_table_clean$Team[row] ,word( model_pred$Away_Team,-1), maxDist = 2)]
    ...
    #NFL_table_clean$Playoffs[row] = ifelse( away_spread <  home_spread, -abs(midpoint), abs(midpoint) )
    NFL_table_clean$Projection[row] = ifelse(away_spread < 0, paste("+",as.character(round(away_spread,1)),sep=""),  paste("-",as.character(round(away_spread,1)),sep=""))
  }
